...
impl Contract {
    ...
    pub fn update_product(
        ...
        self.new_product(product_id.clone(), new_name, new_total_supply, new_price, new_desc);
    }
    ...
    pub fn delete_product(&mut self, product_id: ProductId) {
        ...
        let product_index = self
            ...
            .iter()
            .enumerate()
            .find(|(_, product)| product.1.product_id == product_id)
            .map(|(index, _)| index);
        if let Some(index) = product_index {
            self.products.remove(&(index as u128));
        }
